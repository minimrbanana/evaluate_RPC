...
def edmonds_karp(n, C, s, t, sumC):
	...
	while True:
		...
		if P[t] == -1: # We did not find a path to t			
			return (sum([F[s][i] for i in xrange(n)]), F)

class AnnoGraph:

	def __init__(self, anno, det, style, minCover, minOverlap, maxDistance):
	...
	def decreaseScore(self, score):
		...
		if capacity_change:
			[self.flow, self.F] = edmonds_karp(self.a, self.C, 0, self.a - 1, self.sumC_active)
...
def main():	
	...
	for i in xrange(len(annoIDL)):
		...
		graphs.append(AnnoGraph(annoIDL[i], detIDL[filterIndex], matchingStyle, options.minCover, options.minOverlap, options.maxDistance))
	...
	for i,nextrect in enumerate(allRects):
		...
		if (i == numDet - 1 or score != allRects[i + 1].rect.score):	
			if(changed or i == numDet - 1):
					
				if(lastcd > 0):
					scores.append(lastScore)
					recalls.append(float(lastsf) / float(noAnnotations))
					precs.append(float(lastsf) / float(lastcd))
					fppi.append(float(lastcd - lastsf) / float(len(annoIDL)))
								
				if (cd > 0):
					recalls.append(float(sf) / float(noAnnotations))
					scores.append(score)					
